...
class EventImpactModel:
    ...
    def _preprocess(self):
        ...
        self.events_df["event_date"] = pd.to_datetime(
            self.events_df["observation_date"]
        )
    ...
    def build_association_matrix(self) -> pd.DataFrame:
        ...
        merged["signed_magnitude"] = merged.apply(
            lambda row: (
                ...
                if str(row.get("impact_direction", "positive")).lower() == "positive"
                else -row["impact_magnitude"]
            ),
            axis=1,
        )
    ...
    def calculate_cumulative_event_effect(
        self, indicator_code: str, target_year: int
    ) -> float:
        """Calculates time-lagged, cumulative event effects for a specific target forecast year."""
        merged = pd.merge(
            self.impact_links_df[
                self.impact_links_df["related_indicator"] == indicator_code
            ],
            self.events_df[["id", "name", "event_date"]],
            left_on="parent_id",
            right_on="id",
        ...
        for _, row in merged.iterrows():
            ...
            if target_year >= effective_year:
                direction = (
                    1.0
                    if str(row.get("impact_direction", "positive")).lower()
                    == "positive"
                    else -1.0
                )
