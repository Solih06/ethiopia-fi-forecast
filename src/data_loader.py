...
class DataLoader:
    ...
    def __init__(self, data_dir: Optional[str] = None):
        self.unified_path = (
            os.path.join(data_dir, "ethiopia_fi_unified_data.csv")
            if data_dir
            else UNIFIED_DATA_PATH
        )
        self.ref_path = (
            os.path.join(data_dir, "reference_codes.csv")
            if data_dir
            else REF_CODES_PATH
        )
    ...
    def validate_schema(self, df: pd.DataFrame) -> bool:
        ...
        if invalid_types:
            raise ValueError(
                f"Schema validation failed. Invalid record_types found: {invalid_types}"
            )
        return True
