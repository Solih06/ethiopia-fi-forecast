...
def run_deepened_analysis():
    ...
    if not acc_df.empty:
        ...
        plt.plot(
            acc_df["observation_date"],
            acc_df["value_numeric"],
            marker="o",
            linewidth=2.5,
            color="#1f77b4",
            label="Account Ownership (Access)",
        )
        ...
        plt.title(
            "Ethiopia Account Ownership Trajectory with Major Market Events",
            fontsize=14,
            pad=15,
        )
    ...
    if not mm_df.empty:
        plt.plot(
            mm_df["observation_date"],
            mm_df["value_numeric"],
            marker="s",
            linewidth=2.5,
            color="#2ca02c",
            label="Mobile Money Wallet Ownership (Usage)",
        )
    else:
    ...
    plt.title(
        "Digital Payment Adoption & Mobile Money Scaling Footprint", fontsize=14, pad=15
    )
    plt.xlabel("Year", fontsize=12)
