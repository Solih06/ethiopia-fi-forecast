...
class FIForecaster:
    ...
    def fit_and_forecast(
        ...
        df_ind = (
            self.historical_df[self.historical_df["indicator_code"] == indicator_code]
            .sort_values("year")
            .copy()
        )
        ...
        if len(df_ind) < 2:
            raise ValueError(
                f"Insufficient historical observations for indicator {indicator_code}"
            )
        ...
        for yr in forecast_years:
            ...
            event_effect = 0.0
            if impact_model:
                event_effect = impact_model.calculate_cumulative_event_effect(
                    indicator_code, yr
                )
